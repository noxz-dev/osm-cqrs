-- https://github.com/makinacorpus/ImpOsm2pgRouting/blob/master/pgRouting/10_network.template.sql
...
(
    ...
    bigint
    NOT
    NULL, -- required field, for update
    source_vertex_id
    int,  --required field, for routing
    target_vertex_id
    int,  -- required field, for routing
    geometry
    geometry
...
CREATE
OR REPLACE FUNCTION network_add(
...
CREATE
OR REPLACE FUNCTION network_delete(
    _osm_id bigint -- OpenStreetMap way id, not unique as way are splited into segements
) RETURNS void AS $$
BEGIN
DELETE
...
CREATE
OR REPLACE FUNCTION cost(geometry geometry, tags hstore) RETURNS float AS $$
BEGIN
