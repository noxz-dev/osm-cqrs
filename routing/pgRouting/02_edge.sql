-- https://github.com/makinacorpus/ImpOsm2pgRouting/blob/master/pgRouting/02_edge.sql
-- Combinable versions of array_agg to be able run parallelized queries.

CREATE
OR REPLACE FUNCTION imposm2pgr.array_agg_append(accu int[], id int) RETURNS int[] AS $$
SELECT array_append(accu, id);
$$
LANGUAGE SQL IMMUTABLE PARALLEL SAFE;

CREATE
OR REPLACE FUNCTION imposm2pgr.array_agg_combine(accu1 int[], accu2 int[]) RETURNS int[] AS $$
SELECT accu1 || accu2;
$$
LANGUAGE SQL IMMUTABLE PARALLEL SAFE;

CREATE
OR REPLACE AGGREGATE combinable_array_agg (int) (
...
CREATE
OR REPLACE FUNCTION imposm2pgr.array_agg_append(accu geometry[], id geometry) RETURNS geometry[] AS $$
SELECT array_append(accu, id);
$$
LANGUAGE SQL IMMUTABLE PARALLEL SAFE;

CREATE
OR REPLACE FUNCTION imposm2pgr.array_agg_combine(accu1 geometry[], accu2 geometry[]) RETURNS geometry[] AS $$
SELECT accu1 || accu2;
$$
LANGUAGE SQL IMMUTABLE PARALLEL SAFE;

CREATE
OR REPLACE AGGREGATE combinable_array_agg (geometry) (
    SFUNC = imposm2pgr.array_agg_append,
    COMBINEFUNC = imposm2pgr.array_agg_combine,
    STYPE = geometry[],
    PARALLEL = SAFE
...
CREATE
OR REPLACE FUNCTION imposm2pgr.initialize_network() RETURNS boolean AS $$
BEGIN
    RAISE
NOTICE '% Split ways as segements', timeofday()::timestamp;
    -- Use an intermediate temps table to allow parallel plan not permited by network_add().
    CREATE
TEMP TABLE temp_network AS
SELECT osm_id,
       geometry,
       tags,
...
RAISE
NOTICE '% Insert segements into network', timeofday()::timestamp;
    PERFORM
network_add(
            osm_id,
            geometry,
...
RAISE
NOTICE '% initialize_network done', timeofday()::timestamp;
RETURN true;
END
$$
LANGUAGE plpgsql;
...
CREATE
OR REPLACE FUNCTION imposm2pgr.update_network() RETURNS boolean AS $$
BEGIN
